...
def adam(w, dw, config=None):
  ...
  moment_1 = config["beta1"] * config["v"] + (1 - config["beta1"]) * dw
  moment_2 = config["beta2"] * config["a"] + (1 - config["beta2"]) * dw * dw
  config["t"] += 1
  a = config["learning_rate"] * np.sqrt(1 - config["beta2"] ** config["t"]) / (1 - config["beta1"] ** config["t"])
  next_w = w - a * (moment_1 / (np.sqrt(moment_2) + config["epsilon"]))
  config["a"] = moment_2
  config["v"] = moment_1
